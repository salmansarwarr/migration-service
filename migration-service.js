...
const CONFIG = {
    ...
    PLATFORM_AUTHORITY: new PublicKey("9CgjeM8CfEXXBVMvTfPjbB2iLPNHFCVGgdYRZw9FdjRk"),
    SOL_MINT: new PublicKey('So11111111111111111111111111111111111111112'),
    RPC_URL: process.env.SOLANA_RPC_URL || "https://solana-mainnet.api.syndica.io/api-key/21P91u6oC24BUjduDPBnPEdmPWWz7fmFp3jtMBY52Mgq5j1CE9sjKbUv1TzPZGan2pKeDg289fHqvdP6UK5cAHhyJmuHSLE2qm",
    ...
    MIN_SOL_BALANCE: 0.1, // Minimum SOL to keep operations running
    MAX_RETRIES: 5, // Increased for mainnet reliability
    CONFIRMATION_COMMITMENT: 'finalized', // Stronger finality for mainnet
};
...
class UnifiedMigrationUnlockService {
    ...
    async start() {
        ...
        const balance = await this.connection.getBalance(this.botWallet.publicKey);
        const solBalance = balance / 1e9;

        console.log(`💰 Bot wallet balance: ${solBalance} SOL`);

        if (solBalance < CONFIG.MIN_SOL_BALANCE) {
            throw new Error(`Insufficient balance! Need at least ${CONFIG.MIN_SOL_BALANCE} SOL`);
        }
